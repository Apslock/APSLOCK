...
export default function BuildInnovationPage() {
  ...
  const [headline, setHeadline] = useState("Where the strategy shows up in production.");
  ...
  const [eyebrow, setEyebrow] = useState("Build & Innovation");
  const [eyebrowSize, setEyebrowSize] = useState(1.75); // in rem
  const [eyebrowFont, setEyebrowFont] = useState("font-display");
  const [eyebrowColor, setEyebrowColor] = useState("#0D0D0D");

  const [description, setDescription] = useState("Websites, apps, composable systems, and AI tooling — engineered to the cadence and quality the rest of the business demands. We build the product surface your team is paid to ship against, and leave it in a shape they actually want to inherit.");
  const [descriptionSize, setDescriptionSize] = useState(15); // in px
  const [descriptionColor, setDescriptionColor] = useState("#1A1625");
  const [descWidth, setDescWidth] = useState(520); // in px
  ...
  const [blobColor, setBlobColor] = useState("slate"); // sage, amber, slate, blush, custom
  const [customBlobColor, setCustomBlobColor] = useState("#2a9d8f");
  ...
  useEffect(() => {
    try {
      const getStored = (key: string, fallback: any) => {
        const item = localStorage.getItem(`build_${key}`);
        return item ? JSON.parse(item) : fallback;
      };

      setHeadline(localStorage.getItem("build_headline") || "Where the strategy shows up in production.");
      setHeadlineSize(getStored("headlineSize", 4.8));
      setHeadlineFont(localStorage.getItem("build_headlineFont") || "font-script");
      setHeadlineColor(localStorage.getItem("build_headlineColor") || "#0D0D0D");
      setHeadlineWidth(getStored("headlineWidth", 720));

      setEyebrow(localStorage.getItem("build_eyebrow") || "Build & Innovation");
      setEyebrowSize(getStored("eyebrowSize", 1.75));
      setEyebrowFont(localStorage.getItem("build_eyebrowFont") || "font-display");
      setEyebrowColor(localStorage.getItem("build_eyebrowColor") || "#0D0D0D");

      setDescription(localStorage.getItem("build_description") || "Websites, apps, composable systems, and AI tooling — engineered to the cadence and quality the rest of the business demands. We build the product surface your team is paid to ship against, and leave it in a shape they actually want to inherit.");
      setDescriptionSize(getStored("descriptionSize", 15));
      setDescriptionColor(localStorage.getItem("build_descriptionColor") || "#1A1625");
      ...
      setBlobColor(localStorage.getItem("build_blobColor") || "slate");
      setCustomBlobColor(localStorage.getItem("build_customBlobColor") || "#2a9d8f");
  ...
  return (
    <main className="flex-1 bg-[#F3F1ED] expertise-page relative">
      <GrainBlobs variant="slate" intensity={0.12} animate={true} className="fixed inset-0 z-0 pointer-events-none" />
            ...
            <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-[680px] px-4 md:px-0">
              ...
              <div className="transform-none md:-translate-y-12">
                <ExpertiseHeroCard scrollY={smoothY}
                  ...
                  eyebrow="ECOMMERCE"
                  title="Websites"
                  body="Scalable digital storefronts."
                  icon="layers"
              ...
              <div className="transform-none">
                <ExpertiseHeroCard scrollY={smoothY}
                  ...
                  title="Web & Mobile"
                  body="Native and web applications."
                  icon="target"
              ...
              <div className="transform-none md:translate-y-4">
                <ExpertiseHeroCard scrollY={smoothY}
                  ...
                  eyebrow="MARTECH"
                  title="Ecosystems"
                  body="Integrated marketing platforms."
                  icon="chat"
                  className="h-[260px] md:h-[316px] w-full"
                  rotate={1.7}
                />
